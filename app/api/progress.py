...
async def get_current_progress(
    ...
    try:
        ...
        days_response = supabase.table("roadmap_days").select("day_id, day_number, estimated_minutes").eq("project_id", project_id).order("day_number", desc=False).execute()
        ...
        if current_day:
            ...
            concepts_response = supabase.table("concepts").select("concept_id, order_index, title, estimated_minutes").eq("day_id", current_day["day_id"]).order("order_index", desc=False).execute()
...
async def mark_content_read(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to mark content as read: {str(e)}")
...
async def _check_and_complete_concept_if_ready(supabase: Client, user_id: str, concept_id: str, project_id: str):
    ...
    try:
        ...
        concept_progress_response = supabase.table("user_concept_progress").select("content_read").eq("user_id", user_id).eq("concept_id", concept_id).execute()
        content_read = False
        ...
        if not task_ids and content_read:
            ...
            should_complete = True
            logger.info(f"✅ All tasks completed and content read for concept {concept_id}, auto-completing")
