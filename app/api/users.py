...
async def sync_user(
    ...
    try:
        ...
        existing_user_response = supabase.table("User").select("*").eq("clerk_user_id", clerk_user_id).execute()
        
        if existing_user_response.data and len(existing_user_response.data) > 0:
            # Update existing user
            updated_user_response = supabase.table("User").update({
                "email": email,
                "name": name,
            ...
            # Create new user
            new_user_response = supabase.table("User").insert({
...
async def get_current_user(
    ...
    try:
        ...
        user_response = supabase.table("User").select("*").eq("clerk_user_id", clerk_user_id).execute()
