...
from app.services.file_system import get_file_system_service, FileSystemService
...
def get_container_id(
    ...
    if status != "running":
        raise HTTPException(
            status_code=400, 
            detail=f"Workspace container is not running (status: {status})"
        )
...
def list_files(
    ...
    try:
        ...
        return ListFilesResponse(
            success=True,
            path=path,
            files=[FileItem(**f.to_dict()) for f in files]
        )
...
def create_file(
    ...
    try:
        ...
        logger.info(f"Creating {'directory' if request.is_directory else 'file'}: {request.path} in container {container_id[:12]}")
