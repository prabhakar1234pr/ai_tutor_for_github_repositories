...
def validate_state(state: RoadmapAgentState, required_fields: List[str]) -> None:
    ...
    if missing:
        raise ValueError(
            ...
            f"Available fields: {list(state.keys())}"
        )
...
def calculate_recursion_limit(target_days: int, avg_concepts_per_day: int = 4) -> int:
    ...
    logger.info(f"📊 Calculated recursion limit: {limit} (target_days={target_days}, avg_concepts={avg_concepts_per_day})")
...
def get_error_context(state: RoadmapAgentState) -> Dict[str, Any]:
    ...
    return {
        ...
        "concept_title": (
            state.get("current_concepts", [{}])[state.get("current_concept_index", 0)].get("title", "unknown")
            if state.get("current_concepts") and state.get("current_concept_index", 0) < len(state.get("current_concepts", []))
            else "unknown"
        ),
