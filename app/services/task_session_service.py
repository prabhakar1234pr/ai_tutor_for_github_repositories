...
class TaskSessionService:
    ...
    def __init__(self, supabase: Optional[Client] = None, git_service: Optional[GitService] = None):
        self.supabase = supabase or get_supabase_client()
        self.git_service = git_service or GitService()

    def start_task_session(self, task_id: str, user_id: str, workspace_id: str) -> Dict[str, object]:
        """
        Create or get an existing task session and capture base commit.
        """
        existing = (
            self.supabase.table("task_sessions")
            .select("session_id, task_id, user_id, workspace_id, base_commit, current_commit, started_at, completed_at")
            .eq("task_id", task_id)
            .eq("user_id", user_id)
    ...
    def complete_task_session(self, session_id: str, current_commit: Optional[str] = None) -> Dict[str, object]:
        """
    ...
    def get_diff_for_verification(self, session_id: str) -> Dict[str, object]:
        ...
        diff = self.git_service.git_diff(container_id, base_commit=session["base_commit"], head_commit="HEAD")
        if not diff.get("success"):
            return {"success": False, "error": diff.get("error")}

        return {"success": True, "diff": diff.get("diff", ""), "base_commit": session["base_commit"]}

    def _get_session_by_id(self, session_id: str) -> Optional[Dict[str, object]]:
        response = (
            self.supabase.table("task_sessions")
            .select("session_id, task_id, user_id, workspace_id, base_commit, current_commit, started_at, completed_at")
            .eq("session_id", session_id)
