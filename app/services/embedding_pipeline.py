...
from app.services.qdrant_service import get_qdrant_service, COLLECTION_NAME
...
async def run_embedding_pipeline(
    ...
    try:
        ...
        if api_start_time:
            ...
            logger.info(f"⏱️  [TIMING] Step 2 completed - Cumulative: {cumulative_time:.3f}s | Total from API: {total_time_from_api:.3f}s")
        
        logger.info(f"✅ Step 2/7: Fetched {len(files)} files ({total_size_kb:.1f} KB / {total_size_mb:.2f} MB) in {fetch_duration:.2f}s")
        if fetch_duration > 0:
            logger.info(f"📊 [METRICS] Repository size: {total_size_mb:.2f} MB | Files: {len(files)} | Fetch rate: {total_size_mb / fetch_duration:.2f} MB/s")
        ...
        if api_start_time:
            total_time_from_api = time.time() - api_start_time
            logger.info(f"⏱️  [TIMING] Step 3 completed - Cumulative: {cumulative_time:.3f}s | Total from API: {total_time_from_api:.3f}s")
        
        logger.info(f"✅ Step 3/7: Created {len(chunks)} chunks ({total_tokens:,} total tokens) in {chunk_duration:.2f}s")
        if chunk_duration > 0:
            logger.info(f"📊 [METRICS] Chunking rate: {len(chunks) / chunk_duration:.1f} chunks/s | {total_size_mb / chunk_duration:.2f} MB/s")
        ...
        if api_start_time:
            total_time_from_api = time.time() - api_start_time
            logger.info(f"⏱️  [TIMING] Step 4 completed - Cumulative: {cumulative_time:.3f}s | Total from API: {total_time_from_api:.3f}s")
            logger.info(f"⏱️  [TIMING] ✅ CHUNKS STORED IN SUPABASE - Time from 'Let's start building': {total_time_from_api:.3f}s ({total_time_from_api/60:.2f} minutes)")
        
        logger.info(f"✅ Step 4/7: Stored {len(chunk_ids)} chunks in Supabase in {store_duration:.2f}s")
        logger.info(f"📊 [METRICS] Storage rate: {len(chunk_ids) / store_duration:.1f} chunks/s | {total_size_mb / store_duration:.2f} MB/s" if store_duration > 0 else "")
        ...
        if api_start_time:
            total_time_from_api = time.time() - api_start_time
            logger.info(f"⏱️  [TIMING] Step 5 completed - Cumulative: {cumulative_time:.3f}s | Total from API: {total_time_from_api:.3f}s")
        ...
        if embed_duration > 0:
            logger.info(f"📊 [METRICS] Embedding rate: {len(embeddings) / embed_duration:.1f} embeddings/s | {total_tokens / embed_duration:.0f} tokens/s")
        ...
        if api_start_time:
            total_time_from_api = time.time() - api_start_time
            logger.info(f"⏱️  [TIMING] Step 6 completed - Cumulative: {cumulative_time:.3f}s | Total from API: {total_time_from_api:.3f}s")
            logger.info(f"⏱️  [TIMING] ✅ EMBEDDINGS STORED IN QDRANT - Time from 'Let's start building': {total_time_from_api:.3f}s ({total_time_from_api/60:.2f} minutes)")
        
        logger.info(f"✅ Step 6/7: Upserted {len(embeddings)} embeddings into Qdrant in {qdrant_duration:.2f}s")
        logger.info(f"📊 [METRICS] Qdrant upsert rate: {len(embeddings) / qdrant_duration:.1f} embeddings/s" if qdrant_duration > 0 else "")
        ...
        if api_start_time and total_time_from_api:
            logger.info(f"")
            logger.info(f"=" * 80)
            logger.info(f"⏱️  [TIMING SUMMARY] Total time from 'Let's start building' to completion")
            logger.info(f"=" * 80)
            logger.info(f"   🎯 Total Time: {total_time_from_api:.3f}s ({total_time_from_api/60:.2f} minutes)")
            logger.info(f"   📥 GitHub Fetch: {fetch_duration:.3f}s ({fetch_duration/total_time_from_api*100:.1f}%)")
            logger.info(f"   ✂️  Chunking: {chunk_duration:.3f}s ({chunk_duration/total_time_from_api*100:.1f}%)")
            logger.info(f"   💾 Supabase Storage: {store_duration:.3f}s ({store_duration/total_time_from_api*100:.1f}%)")
            logger.info(f"   🧮 Embedding Generation: {embed_duration:.3f}s ({embed_duration/total_time_from_api*100:.1f}%)")
            logger.info(f"   🔍 Qdrant Storage: {qdrant_duration:.3f}s ({qdrant_duration/total_time_from_api*100:.1f}%)")
            logger.info(f"   ⚙️  Other (status updates, etc.): {total_time_from_api - fetch_duration - chunk_duration - store_duration - embed_duration - qdrant_duration:.3f}s")
            logger.info(f"=" * 80)
        ...
        if total_size_mb > 0 and total_time_from_api and total_time_from_api > 0:
            ...
            logger.info(f"   • Tokens per second: {total_tokens / total_time_from_api:.0f} tokens/s")
            
            # Estimation formula
            logger.info(f"")
            logger.info(f"🔮 [ESTIMATION FORMULA] Based on current processing speed:")
            logger.info(f"   Estimated time (seconds) = Repository Size (MB) / {mb_per_second:.2f}")
            logger.info(f"   Estimated time (minutes) = Repository Size (MB) / {mb_per_second:.2f} / 60")
            logger.info(f"")
            logger.info(f"   Examples:")
            logger.info(f"   • 200 MB repo: ~{200 / mb_per_second:.1f}s ({200 / mb_per_second / 60:.1f} min)")
            logger.info(f"   • 500 MB repo: ~{500 / mb_per_second:.1f}s ({500 / mb_per_second / 60:.1f} min)")
            logger.info(f"   • 1 GB repo: ~{1024 / mb_per_second:.1f}s ({1024 / mb_per_second / 60:.1f} min)")
            
            # Compare actual vs estimated
            from app.utils.time_estimation import estimate_processing_time
            estimated = estimate_processing_time(total_size_mb)
            logger.info(f"")
            logger.info(f"📊 [ACTUAL vs ESTIMATED]")
            logger.info(f"   Actual time: {total_time_from_api:.1f}s ({total_time_from_api/60:.1f} min)")
            logger.info(f"   Estimated time: {estimated['total_seconds']:.1f}s ({estimated['total_minutes']:.1f} min)")
            accuracy = (1 - abs(total_time_from_api - estimated['total_seconds']) / total_time_from_api) * 100
        ...
        if api_start_time:
            ...
            logger.error(f"⏱️  [TIMING] Failed after {error_duration:.2f}s (pipeline) / {total_time_from_api:.2f}s (total from API)")
        else:
