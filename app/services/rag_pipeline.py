...
async def generate_rag_response(
    ...
    logger.info(f"📝 Step 1/5: Generating embedding for query")
    ...
    logger.info(f"✅ Generated query embedding (dim={len(query_embedding)}) in {embed_duration:.3f}s")
    
    # Step 2: Search Qdrant for similar chunks
    logger.info(f"🔍 Step 2/5: Searching Qdrant for similar chunks")
    ...
    logger.debug(f"   Chunk IDs: {chunk_ids[:3]}..." if len(chunk_ids) > 3 else f"   Chunk IDs: {chunk_ids}")
    
    # Step 3: Retrieve chunk content from Supabase
    logger.info(f"💾 Step 3/5: Retrieving chunk content from Supabase")
    ...
    logger.info(f"✅ Retrieved {len(retrieved_chunks)} chunks from Supabase in {supabase_duration:.3f}s")
    
    # Step 4: Build context string
    logger.info(f"📚 Step 4/5: Building context from retrieved chunks")
    context_parts = []
    chunks_used = []
    ...
    logger.debug(f"   Files referenced: {len(set(c['file_path'] for c in retrieved_chunks))}")
    
    # Step 5: Generate response using Groq API
    logger.info(f"🤖 Step 5/5: Generating response with Groq API")
