        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Successfully sanitized JSON ({expected_type}) using {self.model}"
                    )
                    return parsed
                    ...
                    raise ValueError(f"Sanitizer returned invalid JSON: {e}") from e
            ...
            raise ValueError(f"JSON sanitization failed: {error_msg}") from e

        except Exception as e:
            logger.error(f"❌ JSON sanitization failed: {e}", exc_info=True)
            raise ValueError(f"JSON sanitization failed: {e}") from e
