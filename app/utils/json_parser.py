        ...
        try:
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    logger.warning("⚠️  Cannot use async sanitizer in sync context, attempting basic parsing...")
        ...
        try:
            ...
            try:
                ...
                if not is_markdown_or_code and settings.groq_sanitizer_enabled:
                    try:
                        logger.warning(f"⚠️  Basic JSON parsing failed, attempting sanitization...")
                        sanitizer = _get_sanitizer()
                        try:
                            loop = asyncio.get_event_loop()
                            if loop.is_running():
